...
func (p *nodeProcess) awaitExit() {
	if err := p.cmd.Wait(); err != nil {
		p.log.Debug("node returned error on wait", zap.String("node-name", p.name), zap.Error(err))
	}
...
func (p *nodeProcess) Stop(ctx context.Context) int {
	...
	select {
	case <-ctx.Done():
		p.log.Warn("context cancelled while waiting for node to stop", zap.String("node-name", p.name))
...
func killDescendants(pid int32, log logging.Logger) {
	...
	for _, proc := range procs {
		...
		if err := proc.Kill(); err != nil {
			log.Warn("error killing process", zap.Int32("process-ID", proc.Pid), zap.Error(err))
		}
	}
}
