...
An example of such a github action can be found at `programs/github` **TODO**.
...
7. Install the service account, role and roleconfigs for the network runner pod: `kubectl apply -f programs/k8s/svc-rbac.yaml`.
8. Build this app as a pod: `docker build -f ./programs/k8s/Dockerfile -t k8s-netrunner:alpha .`. If you use a different app name than `k8s-netrunner` and/or tag `alpha`, the `programs/k8s/simple-network-pod.yaml` file needs to be updated accordingly.
9. Deploy the pod to your local cluster via `kubectl apply -f programs/k8s/simple-netrunner-pod.yaml`. This picks the image built in 8. to deploy as a pod. Then from within this pod it starts all other nodes.
