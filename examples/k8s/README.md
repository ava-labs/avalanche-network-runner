...
8. Build this app as a pod: `docker build -f ./examples/k8s/Dockerfile -t nettool:alpha .`. If you use a different app name than `k8s-netrunner` and/or tag `alpha`, the `examples/k8s/simple-network-pod.yaml` file needs to be updated accordingly.
